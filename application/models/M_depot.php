<?php
  class M_depot extends  MY_Model{
      ...
      public function get_list_depot(){
        ...
        AND  (cd.id_depot NOT IN(SELECT cd2.id_depot FROM circuit_depot cd2 JOIN( circuit c2 JOIN type_dossier td2 ON(c2.id_type_dossier=td2.id_type_dossier))ON(cd2.id_circuit=c2.id_circuit) WHERE cd2.id_depot=cd.id_depot AND c2.ordre=c.ordre-1 AND cd2.etat IN ('en_attente','rejeté') ) OR d.depot_central=1)";		    
